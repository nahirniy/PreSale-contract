...
describe("TokenSale", function () {
  async function deploy() {
    ...
    const shop = await TokenSale.deploy(owner, usdt.target, mockV3Aggregator.target, tokenPrice);
  ...
  it("correct work function that canculate eth and usdt for token", async function () {
  ...
  it("correct work of ustd, token, and ether balances", async function () {
    ...
    expect(await shop.tokenBalance()).to.eq(await token.totalSupply()); // since token in the vesting
  });
  ...
  it("can claim token after vesting period", async function () {
    ...
    expect(await token.balanceOf(shop.target)).to.eq((await token.totalSupply()) - tokenAmount);
